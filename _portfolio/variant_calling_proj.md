...
[bulk](plant_rnaseq_proj) [RNA-seq](aml_proj) and
[spatial](spatial_visium_proj) pipelines quantify *expression*, this one
...
plus group-specific) written to a ground-truth VCF — so the whole DAG, including
a recovery self-check, runs in under a minute with no download. The **real
...
> drop-in sibling — both deliberate, documented trade-offs that keep the project
> laptop-reproducible.
