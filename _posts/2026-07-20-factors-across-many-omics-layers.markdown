...
**The problem.** When a study measures several modalities across several groups of cells, you want the major axes of variation that structure the whole dataset, and, above all, to know which modality each axis is driven by. Analysing each layer separately misses shared structure; naïve concatenation loses the ability to attribute variation to its source.
